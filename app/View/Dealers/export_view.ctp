...
if (!empty($all))
{
	...
	$worksheet->writeString(0, 12, 'Dealer_Published', $format_bold);
	$worksheet->writeString(0, 13, 'Dealer_Approved', $format_bold);
	$i = 1;
	foreach ($all as $dealerID=>$dealer)
	{
		...
		$worksheet->writeString($i, 12, $dealer['Dealer']['published']);
		$worksheet->writeString($i, 13, $dealer['Dealer']['approved']);
		$i++;
	}
}

// Creating the 2nd worksheet for Dealers who own Zipcodes
$worksheet =& $workbook->addWorksheet('Dealers Zips');

// The actual data
if (!empty($dealers_zips))
{
	$worksheet->writeString(0, 0, 'Dealer_Number', $format_bold);
	$worksheet->writeString(0, 1, 'Zipcode', $format_bold);
	$i = 1;
	foreach ($dealers_zips as $dealerID=>$dealer)
	{
		$worksheet->writeString($i, 0, $dealer['Dealer']['dealer_number']);
		$worksheet->writeString($i, 1, $dealer['dz']['zipcode_id']);
		$i++;
	}
}
// Let's send the file
